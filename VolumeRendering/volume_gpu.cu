...
__global__ void render_ray_gpu(Volume_model volume, View view, Raycaster raycaster, uchar4 dev_buffer[]) {
...
extern float render_volume_gpu(uchar4 *buffer, View current_view, Raycaster current_raycaster) {
	...
	render_ray_gpu<<<num_blocks, threads_per_block>>>(volume_model, current_view, current_raycaster, dev_buffer);
